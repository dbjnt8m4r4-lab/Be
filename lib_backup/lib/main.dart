...
void main() async {
  ...
  // Initialize Firebase (best effort). If it fails, continue in offline mode.
  try {
    await Firebase.initializeApp(
      options: DefaultFirebaseOptions.currentPlatform,
    );
  } catch (e) {
    // Ignore Firebase errors so the app can still run fully offline.
  }

  // Initialize local notifications
  await NotificationService().init();

  // Get shared preferences for initial route decision
  final prefs = await SharedPreferences.getInstance();

  runApp(
    DevicePreview(
      enabled: false, // Disable DevicePreview in production
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      routes: {
        ...
        '/welcome-description': (context) =>
        const WelcomeDescriptionScreen(),
